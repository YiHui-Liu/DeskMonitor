...
static const char *CONTROL_PAGE =
    ...
    "<label>Carousel seconds / 轮播秒数 <input name=\"carousel_interval_sec\" type=\"number\" min=\"5\" "
    "max=\"3600\"></label><br>"
    "<label>Sensor read seconds / 传感器读取秒数 <input name=\"sensor_read_interval_sec\" type=\"number\" min=\"1\" "
    "max=\"3600\"></label><br>"
    "<label>History retention hours / 历史保留小时 <input name=\"sensor_history_retention_hours\" type=\"number\" "
    ...
    "<th>Name / 名称</th><th>Sensor / 传感器</th><th>Address / 地址</th><th>Status / 状态</th><th>Reading / "
    "读数</th><th>Unit / 单位</th>"
    "</tr></thead><tbody id=\"sensor_status\"><tr><td colspan=\"6\">Loading / 加载中</td></tr></tbody></table>"
    "</section>"
    "<section><h2>System / 系统</h2>"
    "<h3>Memory / 内存</h3><table border=\"1\" cellpadding=\"4\"><tbody id=\"memory_status\"><tr><td>Loading / "
    "加载中</td></tr></tbody></table>"
    "<h3>Storage / 存储</h3><table border=\"1\" cellpadding=\"4\"><tbody id=\"storage_status\"><tr><td>Loading / "
    "加载中</td></tr></tbody></table>"
    "<h3>Partitions / 分区</h3><table border=\"1\" cellpadding=\"4\"><thead><tr><th>Label / "
    "标签</th><th>Type</th><th>Subtype</th><th>Address / 地址</th><th>Size / 大小</th></tr></thead><tbody "
    "id=\"partition_status\"><tr><td colspan=\"5\">Loading / 加载中</td></tr></tbody></table>"
    "</section>"
    "<script>"
    ...
    "tr>`).join('');document.getElementById('sensor_status').innerHTML=rows||'<tr><td colspan=\"6\">No sensors / "
    "无传感器</td></tr>';const m=d.system.memory;document.getElementById('memory_status').innerHTML=`<tr><th>Free / "
    "可用</th><td>${bytes(m.free_heap_bytes)}</td></tr><tr><th>Minimum free / "
    "最小可用</th><td>${bytes(m.minimum_free_heap_bytes)}</td></tr><tr><th>Largest block / "
    "最大块</th><td>${bytes(m.largest_free_block_bytes)}</td></tr>`;const "
    "s=d.system.storage;document.getElementById('storage_status').innerHTML=`<tr><th>Label / "
    "标签</th><td>${s.label}</td></tr><tr><th>Status / 状态</th><td>${s.status}</td></tr><tr><th>Used / "
    "已用</th><td>${bytes(s.used_bytes)} / ${bytes(s.total_bytes)}</td></tr><tr><th>Free / "
    "可用</th><td>${bytes(s.free_bytes)}</td></"
    ...
    "tr>`).join('')||'<tr><td colspan=\"5\">No partitions / 无分区</td></tr>';};"
    ...
    "fetch('/api/"
    "config',{method:'POST',body:JSON.stringify(b)});document.getElementById('result').textContent=r.ok?'Saved / "
    "已保存':'Save failed / 保存失败';};"
    "document.getElementById('qweather_test').onclick=async()=>{const "
    "out=document.getElementById('qweather_result');out.textContent='Testing / 测试中...';const r=await "
    "fetch('/api/qweather/"
