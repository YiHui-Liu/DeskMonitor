...
static const char *CONTROL_PAGE =
    ...
    "<button id=\"qweather_test\" type=\"button\">Test / 测试</button><br>"
    ...
    "</section>"
    "<section><h2>System / 系统</h2>"
    "<h3>Memory / 内存</h3><table border=\"1\" cellpadding=\"4\"><tbody id=\"memory_status\"><tr><td>Loading / 加载中</td></tr></tbody></table>"
    "<h3>Storage / 存储</h3><table border=\"1\" cellpadding=\"4\"><tbody id=\"storage_status\"><tr><td>Loading / 加载中</td></tr></tbody></table>"
    "<h3>Partitions / 分区</h3><table border=\"1\" cellpadding=\"4\"><thead><tr><th>Label / 标签</th><th>Type</th><th>Subtype</th><th>Address / 地址</th><th>Size / 大小</th></tr></thead><tbody id=\"partition_status\"><tr><td colspan=\"5\">Loading / 加载中</td></tr></tbody></table>"
    "</section>"
    "<script>"
    "const $=n=>document.querySelector('[name='+n+']');"
    "const bytes=n=>n>=1048576?(n/1048576).toFixed(1)+' MiB':n>=1024?(n/1024).toFixed(1)+' KiB':n+' B';"
    "async function loadDiagnostics(){const r=await fetch('/api/diagnostics');const d=await r.json();const rows=d.quantities.map(x=>`<tr><td>${x.name}</td><td>${x.sensor}</td><td>${x.address}</td><td>${x.status}</td><td>${x.reading}</td><td>${x.unit}</td></tr>`).join('');document.getElementById('sensor_status').innerHTML=rows||'<tr><td colspan=\"6\">No sensors / 无传感器</td></tr>';const m=d.system.memory;document.getElementById('memory_status').innerHTML=`<tr><th>Free / 可用</th><td>${bytes(m.free_heap_bytes)}</td></tr><tr><th>Minimum free / 最小可用</th><td>${bytes(m.minimum_free_heap_bytes)}</td></tr><tr><th>Largest block / 最大块</th><td>${bytes(m.largest_free_block_bytes)}</td></tr>`;const s=d.system.storage;document.getElementById('storage_status').innerHTML=`<tr><th>Label / 标签</th><td>${s.label}</td></tr><tr><th>Status / 状态</th><td>${s.status}</td></tr><tr><th>Used / 已用</th><td>${bytes(s.used_bytes)} / ${bytes(s.total_bytes)}</td></tr><tr><th>Free / 可用</th><td>${bytes(s.free_bytes)}</td></tr>`;document.getElementById('partition_status').innerHTML=d.system.partitions.map(p=>`<tr><td>${p.label}</td><td>${p.type}</td><td>${p.subtype}</td><td>0x${p.address.toString(16)}</td><td>${bytes(p.size_bytes)}</td></tr>`).join('')||'<tr><td colspan=\"5\">No partitions / 无分区</td></tr>';};"
    ...
    "document.getElementById('qweather_test').onclick=async()=>{const out=document.getElementById('qweather_result');out.textContent='Testing / 测试中...';const r=await fetch('/api/qweather/test',{method:'POST',body:JSON.stringify({qweather_api_key:$('qweather_api_key').value,qweather_location:$('qweather_location').value})});const d=await r.json();out.textContent=JSON.stringify(d,null,2);};"
...
static void qweather_fetch_task(void *arg)
{
    ...
    request->err =
        fetch_qweather_now(request->api_key, request->location, &request->http_status, &request->body);
...
static esp_err_t qweather_test_post_handler(httpd_req_t *req)
{
    ...
    bool inputs_valid = cJSON_IsString(api_key) && valid_header_value(api_key->valuestring) &&
                        cJSON_IsString(location) && valid_location_value(location->valuestring);
    ...
    if (xTaskCreate(qweather_fetch_task, "qwfetch", QWEATHER_TASK_STACK, request,
                    QWEATHER_TASK_PRIORITY, NULL) != pdPASS) {
